...
There is a fixed external deadline (September), and the project is
actively being shown to internship recruiters before that deadline,
which makes development order a real decision rather than an arbitrary
choice.
...
the Raft skeleton next (leader election in isolation, CV-relevant and
independently testable), or build the Storage crate next.
...
- Doing the FUSE skeleton first (rather than Storage or Raft first)
  ...
  early, demoable mount and surfaced the handler decomposition design
  (ADR-003) before any storage or consensus complexity was introduced,
  which is useful given the project is visible to recruiters
  mid-development — a partially complete but clearly structured repo
  (with explicit `todo!()` markers and ADRs) is considered acceptable
  and still better than an empty one.
- Raft was not moved ahead of Storage despite being the most
  CV-relevant subsystem, because an in-memory-log Raft skeleton built
  now would still need a later integration pass to swap in real
  WAL-backed persistence once Storage exists — better to resolve
  Storage first and avoid that rework.
